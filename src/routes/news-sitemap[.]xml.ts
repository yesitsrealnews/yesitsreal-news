  ...
  server: {
    handlers: {
      GET: async () => {
        ...
        const items = STORIES.filter((s) => s.status === "published" && !s.sponsored).filter(
          (s) => +new Date(s.publishedAt) >= cutoff || +new Date(s.updatedAt || s.publishedAt) >= cutoff,
        );
        const listed = items.length ? items : STORIES.filter((s) => s.status === "published" && !s.sponsored).slice(0, 20);
