...
async function fetchFeedXml(url: string): Promise<string | null> {
  try {
    ...
    if (!/<rss|<feed|<rdf/i.test(text.slice(0, 400))) return null;
    return text;
  } catch {
    return null;
  }
...
export async function pullRssFeeds(limit = 24): Promise<{ hits: RssHit[]; scanned: number; failed: string[] }> {
  const feeds = RSS_FEEDS.slice(0, limit);
  const failed: string[] = [];
  const hits: RssHit[] = [];
  const seen = new Set<string>();

  for (let i = 0; i < feeds.length; i += 6) {
    const batch = feeds.slice(i, i + 6);
    const xmls = await Promise.all(batch.map((f) => fetchFeedXml(f.url)));
  ...
  return { hits: hits.slice(0, 40), scanned: feeds.length, failed };
}
